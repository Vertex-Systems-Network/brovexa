...
exports.SourceTransportResponseReceiptSchema = void 0;
exports.parseSourceTransportResponseReceipt = parseSourceTransportResponseReceipt;
const node_url_1 = require("node:url");
const zod_1 = require("zod");
const source_transport_1 = require("./source-transport");
const source_transport_hop_chain_1 = require("./source-transport-hop-chain");
...
const NonNegativeSafeIntegerSchema = zod_1.z.number().int().min(0).max(Number.MAX_SAFE_INTEGER);
const TerminalHttpStatusSchema = zod_1.z
    .number()
    .int()
    .min(100)
    .max(599)
...
exports.SourceTransportResponseReceiptSchema = zod_1.z
    .object({
    ...
    hopChain: source_transport_hop_chain_1.SourceTransportHopChainSchema,
    hopAdmissions: zod_1.z.array(source_transport_1.SourceTransportAdmissionDecisionSchema).min(1).max(11),
    final: zod_1.z
        .object({
        ...
        bodyDigest: zod_1.z
            .object({
            algorithm: zod_1.z.literal('sha256'),
            value: zod_1.z.string().regex(/^[0-9a-f]{64}$/),
        })
            .strict(),
    })
        .strict(),
    bodyIncluded: zod_1.z.literal(false),
})
    .strict()
    .superRefine((receipt, ctx) => {
    const chain = receipt.hopChain;
    if (chain.sourceRequestId !== receipt.sourceRequestId ||
    ...
    for (let index = 0; index < alignedCount; index += 1) {
        ...
        if (admission.sourceRequestId !== receipt.sourceRequestId ||
...
function parseSourceTransportResponseReceipt(rawReceipt) {
    return exports.SourceTransportResponseReceiptSchema.parse(rawReceipt);
}
//# sourceMappingURL=source-transport-result.js.map