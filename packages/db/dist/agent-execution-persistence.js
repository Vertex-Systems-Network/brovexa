...
exports.AgentExecutionPlanError = void 0;
exports.persistAgentExecutionPlan = persistAgentExecutionPlan;
exports.getAgentExecutionPlanEnvelope = getAgentExecutionPlanEnvelope;
const agent_persistence_1 = require("./agent-persistence");
const client_1 = require("./client");
class AgentExecutionPlanError extends Error {
    code;
...
exports.AgentExecutionPlanError = AgentExecutionPlanError;
...
function readSafeIntegerField(value, field) {
...
function assertIdentifier(value, field) {
    if (!isNonEmptyString(value) || value.length > 128) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `${field} must be a non-empty identifier no longer than 128 characters.`);
    }
}
function assertUniqueIdentifiers(values, field, maxItems) {
    if (values.length > maxItems || values.some((value) => !isNonEmptyString(value) || value.length > 512)) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `${field} contains an invalid identifier or exceeds ${maxItems} items.`);
    }
    if (new Set(values).size !== values.length) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `${field} must not contain duplicate identifiers.`);
    }
}
function assertInputBudget(budget, field) {
    for (const budgetField of additiveBudgetFields) {
        const value = budget[budgetField];
        if (!Number.isSafeInteger(value) || value < 0) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `${field}.${budgetField} must be a non-negative safe integer.`);
        }
    }
    if (!Number.isInteger(budget.maxConcurrency) || budget.maxConcurrency !== 1) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `${field}.maxConcurrency must equal 1 in the current bounded execution foundation.`);
    }
}
function assertPlanShape(input) {
    ...
    if (input.handoffId !== undefined)
        assertIdentifier(input.handoffId, 'handoffId');
    if (input.orchestratorKey !== 'agent.control.orchestrator') {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', 'Execution plans must use agent.control.orchestrator.');
    }
    ...
    if (!Number.isInteger(input.maxParallelism) ||
    ...
    for (const step of input.steps) {
        ...
        if (!/^[a-z][a-z0-9_.-]{0,127}$/.test(step.key)) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `Step key ${step.key} is not a canonical execution identifier.`);
        }
        if (!/^agent\.[a-z0-9_.-]+$/.test(step.agentKey) || step.agentKey === 'agent.control.orchestrator') {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `Step ${step.key} must target a non-orchestrator canonical agent key.`);
        }
        ...
        if (step.policyRefs.length < 1) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `Step ${step.key} requires at least one policy reference.`);
        }
    ...
    for (const step of input.steps) {
        for (const dependency of step.dependencies) {
            if (dependency === step.key || !stepKeys.has(dependency)) {
                throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', `Step ${step.key} has an invalid dependency: ${dependency}.`);
            }
        }
    }
    ...
    for (const step of input.steps) {
        if (!visit(step.key)) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_INPUT_INVALID', 'Execution plan dependencies must form an acyclic DAG.');
        }
    }
}
function parseDefinition(row) {
    ...
    if (row.status !== 'approved' ||
        ...
        throw new AgentExecutionPlanError('AGENT_EXECUTION_DEFINITION_NOT_APPROVED', `Agent definition ${row.agent_key}@${row.version} is not an approved immutable execution target.`);
    }
    if (specification.autonomyTier !== row.autonomy_tier ||
        ...
        !skillVersions ||
        !budget) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_DEFINITION_INVALID', `Agent definition ${row.agent_key}@${row.version} has an invalid or inconsistent execution specification.`);
    }
    return {
        id: row.id,
        key: row.agent_key,
...
function parseContextReceipt(row, input) {
    ...
    if (row.user_id !== input.userId ||
        ...
        !canonicalRefs ||
        !rawMemoryRefs) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_CONTEXT_MISMATCH', `Context receipt ${input.contextReceiptId} does not match the requested orchestrator execution scope.`);
    }
    ...
    for (const candidate of rawMemoryRefs) {
        if (!isRecord(candidate) || !isNonEmptyString(candidate.memoryId) || !isNonEmptyString(candidate.namespace)) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_CONTEXT_MISMATCH', `Context receipt ${input.contextReceiptId} contains a malformed memory reference.`);
        }
        const memoryId = candidate.memoryId.trim();
        if (memoryRefs.has(memoryId)) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_CONTEXT_MISMATCH', `Context receipt ${input.contextReceiptId} contains duplicate memory reference ${memoryId}.`);
        }
...
function assertSubset(values, allowed, code, messagePrefix) {
    for (const value of values) {
        if (!allowed.has(value))
            throw new AgentExecutionPlanError(code, `${messagePrefix}: ${value}.`);
    }
}
function assertBudgetWithin(requested, allowed, label) {
    for (const field of additiveBudgetFields) {
        if (requested[field] > allowed[field]) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_BUDGET_EXCEEDED', `${label}.${field} exceeds the approved AgentDefinition budget.`);
        }
    }
    if (requested.maxConcurrency > allowed.maxConcurrency) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_BUDGET_EXCEEDED', `${label}.maxConcurrency exceeds the approved AgentDefinition budget.`);
    }
}
function addBudgetValue(current, value, field) {
    const next = current + value;
    if (!Number.isSafeInteger(next)) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_BUDGET_EXCEEDED', `Aggregate ${field} exceeds safe integer accounting limits.`);
    }
    return next;
}
async function assertCurrentWorkspaceRead(client, workspaceId, userId) {
    ...
    if (!result.rows[0]?.allowed) {
        throw new AgentExecutionPlanError('AGENT_EXECUTION_AUTHORIZATION_REQUIRED', 'Current active workspace.read authorization is required to persist an execution plan.');
    }
}
function createPlanEnvelope(input) {
    return {
        id: input.id,
        workspaceId: input.workspaceId,
        userId: input.userId,
...
function createQueuedRunEnvelope(input, definition) {
    return {
        id: input.runId,
        workspaceId: input.workspaceId,
...
async function persistAgentExecutionPlan(pool, input) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        ...
        const existingPlan = await client.query(`SELECT run_id, envelope = $2::jsonb AS same_plan
       FROM agent_execution_plans
       WHERE id = $1`, [input.id, JSON.stringify(planEnvelope)]);
        if (existingPlan.rows[0]) {
            if (!existingPlan.rows[0].same_plan || existingPlan.rows[0].run_id !== input.runId) {
                throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_PLAN_ID_CONFLICT', `Execution plan ${input.id} already exists with different content or scope.`);
            }
            const run = await client.query(`SELECT envelope FROM agent_runs WHERE workspace_id = $1 AND id = $2`, [input.workspaceId, input.runId]);
            const runEnvelope = run.rows[0]?.envelope;
            if (!runEnvelope) {
                throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_PLAN_PROJECTION_MISMATCH', `Execution plan ${input.id} exists without its canonical AgentRun.`);
            }
            return {
                planId: input.id,
                runId: input.runId,
        ...
        const runPlan = await client.query(`SELECT id FROM agent_execution_plans WHERE workspace_id = $1 AND run_id = $2`, [input.workspaceId, input.runId]);
        if (runPlan.rows[0]) {
            throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_RUN_ALREADY_PLANNED', `AgentRun ${input.runId} already belongs to execution plan ${runPlan.rows[0].id}.`);
        }
        const existingRun = await client.query('SELECT id FROM agent_runs WHERE id = $1', [input.runId]);
        if (existingRun.rows[0]) {
            throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_RUN_ID_CONFLICT', `AgentRun ${input.runId} already exists without the requested execution plan.`);
        }
        const contextResult = await client.query(`SELECT
         user_id,
         run_scope_id,
         agent_definition_id,
         agent_key,
       ...
       FOR SHARE`, [input.contextReceiptId, input.workspaceId]);
        const contextRow = contextResult.rows[0];
        if (!contextRow) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_CONTEXT_NOT_FOUND', `Context receipt ${input.contextReceiptId} was not found in workspace ${input.workspaceId}.`);
        }
        if (input.createdAt < contextRow.created_at) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_CONTEXT_MISMATCH', 'Execution plan creation time cannot precede its ContextReceipt.');
        }
        const context = parseContextReceipt(contextRow, input);
        const orchestratorResult = await client.query(`SELECT id, agent_key, version, status, autonomy_tier, requires_human_approval, specification
       FROM agent_definitions
       WHERE id = $1 AND agent_key = $2 AND version = $3
       FOR SHARE`, [contextRow.agent_definition_id, input.orchestratorKey, input.orchestratorVersion]);
        const orchestratorRow = orchestratorResult.rows[0];
        if (!orchestratorRow) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_DEFINITION_NOT_APPROVED', `Orchestrator definition ${input.orchestratorKey}@${input.orchestratorVersion} is unavailable.`);
        }
        ...
        if (orchestrator.key !== 'agent.control.orchestrator' ||
            ...
            throw new AgentExecutionPlanError('AGENT_EXECUTION_DEFINITION_INVALID', 'The canonical orchestrator must be an approved deterministic-only T2 definition.');
        }
        if (orchestrator.requiresHumanApproval) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_REVIEW_REQUIRED', 'The orchestrator requires a human approval artifact not implemented in this slice.');
        }
        if (input.maxParallelism > orchestrator.budget.maxConcurrency) {
            throw new AgentExecutionPlanError('AGENT_EXECUTION_BUDGET_EXCEEDED', 'Plan maxParallelism exceeds the orchestrator concurrency budget.');
        }
        const aggregate = {
            maxTokens: 0,
            maxSearches: 0,
        ...
        for (const step of input.steps) {
         ...
         FOR SHARE`, [step.agentKey, step.agentVersion]);
            ...
            if (!definitionRow) {
                throw new AgentExecutionPlanError('AGENT_EXECUTION_DEFINITION_NOT_APPROVED', `Specialist definition ${step.agentKey}@${step.agentVersion} is unavailable.`);
            }
            const definition = parseDefinition(definitionRow);
            if (definition.requiresHumanApproval || definition.autonomyTier === 'T4') {
                throw new AgentExecutionPlanError('AGENT_EXECUTION_REVIEW_REQUIRED', `Step ${step.key} requires a human approval artifact not implemented in this slice.`);
            }
            assertSubset(step.toolKeys, new Set(definition.allowedTools), 'AGENT_EXECUTION_TOOL_NOT_ALLOWED', `Step ${step.key} requests an unapproved tool`);
            assertSubset(step.commandKeys, new Set(definition.allowedCommands), 'AGENT_EXECUTION_COMMAND_NOT_ALLOWED', `Step ${step.key} requests an unapproved command`);
            assertSubset(step.policyRefs, context.policyRefs, 'AGENT_EXECUTION_SCOPE_BROADENED', `Step ${step.key} broadens policy scope`);
            assertSubset(step.canonicalRefs, context.canonicalRefs, 'AGENT_EXECUTION_SCOPE_BROADENED', `Step ${step.key} broadens canonical scope`);
            for (const memoryId of step.memoryRefs) {
                const memoryRef = context.memoryRefs.get(memoryId);
                if (!memoryRef || memoryRef.status !== 'active') {
                    throw new AgentExecutionPlanError('AGENT_EXECUTION_SCOPE_BROADENED', `Step ${step.key} requests memory ${memoryId} outside the active ContextReceipt.`);
                }
                if (!definition.memoryReadScopes.some((scope) => memoryScopeMatches(scope, memoryRef.namespace))) {
                    throw new AgentExecutionPlanError('AGENT_EXECUTION_MEMORY_SCOPE_NOT_ALLOWED', `Step ${step.key} cannot read memory ${memoryId} under its AgentDefinition scope.`);
                }
            }
        ...
        await client.query(`INSERT INTO agent_execution_plans (
         id,
         workspace_id,
         user_id,
         run_id,
...
async function getAgentExecutionPlanEnvelope(pool, workspaceId, planId) {
    const result = await pool.query(`SELECT envelope
     FROM agent_execution_plans
     WHERE workspace_id = $1 AND id = $2`, [workspaceId, planId]);
    return result.rows[0]?.envelope ?? null;
}
//# sourceMappingURL=agent-execution-persistence.js.map