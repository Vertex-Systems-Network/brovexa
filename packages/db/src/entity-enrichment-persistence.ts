...
function normalizePgSeconds(value:number|string|null,field:string):number|null { if(value===null) return null; const n=typeof value==='number'?value:Number(value); if(!Number.isSafeInteger(n)||n<0) fail('ENTITY_ENRICHMENT_INPUT_INVALID',`${field} contains an invalid persisted duration.`); return n; }
...
function toDomain(row:DomainEvidenceRow):PersistedBusinessDomainEvidence {
  ...
  return { evidenceId:row.id,workspaceId:row.workspace_id,canonicalBusinessId:row.canonical_business_id,normalizedDomain:row.normalized_domain,kind:row.kind,effect:row.effect,sourceKey:row.source_key,sourceReferenceIds:Object.freeze([...row.source_reference_ids]),sourcePolicySnapshot:Object.freeze({policyId:row.source_policy_id,policyVersion:row.source_policy_version}),sourceAdmissionDecisionRef:row.source_admission_decision_ref,sourceAdmissionDecision:'allow',storageClass:row.storage_class,retention:Object.freeze({retentionTtlSeconds:normalizePgSeconds(row.retention_ttl_seconds,'retention_ttl_seconds'),deletionRequired:row.deletion_required,refreshAfterSeconds:normalizePgSeconds(row.refresh_after_seconds,'refresh_after_seconds')}),observedAt:row.observed_at,recordedAt:row.recorded_at,purgedAt:null,purgeReasonCode:null,createdAt:row.created_at };
}
...
export async function persistBusinessDomainEvidence(pool:Pool,input:PersistBusinessDomainEvidenceInput):Promise<EntityEnrichmentPersistenceResult<PersistedBusinessDomainEvidence>> {
  ...
  assertRetention(input.retention); assertDate(input.observedAt,'observedAt'); assertDate(input.recordedAt,'recordedAt'); if(input.recordedAt.getTime()<input.observedAt.getTime()) fail('ENTITY_ENRICHMENT_INPUT_INVALID','recordedAt cannot precede observedAt.');
  ...
  const row=existing.rows[0]; if(!row||row.purged_at!==null||row.canonical_business_id!==input.canonicalBusinessId||row.normalized_domain!==input.normalizedDomain||row.kind!==input.kind||row.effect!==input.effect||row.source_key!==input.sourceKey||!sameArray(row.source_reference_ids,refs)||row.source_policy_id!==input.sourcePolicySnapshot.policyId||row.source_policy_version!==input.sourcePolicySnapshot.policyVersion||row.source_admission_decision_ref!==input.sourceAdmissionDecisionRef||row.source_admission_decision!=='allow'||row.storage_class!==input.storageClass||normalizePgSeconds(row.retention_ttl_seconds,'retention_ttl_seconds')!==input.retention.retentionTtlSeconds||row.deletion_required!==input.retention.deletionRequired||normalizePgSeconds(row.refresh_after_seconds,'refresh_after_seconds')!==input.retention.refreshAfterSeconds||row.observed_at.getTime()!==input.observedAt.getTime()||row.recorded_at.getTime()!==input.recordedAt.getTime()) fail('DOMAIN_EVIDENCE_ID_CONFLICT',`Domain evidence ${input.evidenceId} conflicts with existing durable content.`);
