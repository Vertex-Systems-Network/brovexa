...
exports.persistMemoryRecord = persistMemoryRecord;
exports.getMemoryRecordEnvelope = getMemoryRecordEnvelope;
exports.persistEvalResult = persistEvalResult;
exports.getEvalResultEnvelope = getEvalResultEnvelope;
...
async function persistMemoryRecord(pool, input) {
    assertConfidence(input.confidence);
    const inserted = await pool.query(`INSERT INTO memory_records (
       id,
       version,
       revision_parent_id,
       namespace,
    ...
    if (!existing.rows[0]?.same_memory) {
        throw new agent_persistence_1.AgentPersistenceConflictError('MEMORY_RECORD_ID_CONFLICT', `Memory record ${input.id} already exists with different content, lineage or scope.`);
    }
    return input.id;
}
async function getMemoryRecordEnvelope(pool, workspaceId, memoryId) {
    const result = await pool.query(`SELECT envelope
     FROM memory_records
     WHERE workspace_id = $1 AND id = $2`, [workspaceId, memoryId]);
    return result.rows[0]?.envelope ?? null;
}
async function persistEvalResult(pool, input) {
    assertConfidence(input.confidence);
    const inserted = await pool.query(`INSERT INTO agent_eval_results (
       id,
       workspace_id,
       evaluator_run_id,
       subject_run_id,
    ...
    if (!existing.rows[0]?.same_evaluation) {
        throw new agent_persistence_1.AgentPersistenceConflictError('EVAL_RESULT_ID_CONFLICT', `Evaluation result ${input.id} already exists with different content or scope.`);
    }
    return input.id;
}
async function getEvalResultEnvelope(pool, workspaceId, evaluationId) {
    const result = await pool.query(`SELECT envelope
     FROM agent_eval_results
     WHERE workspace_id = $1 AND id = $2`, [workspaceId, evaluationId]);
    return result.rows[0]?.envelope ?? null;
}
//# sourceMappingURL=memory-eval-persistence.js.map