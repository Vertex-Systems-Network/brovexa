...
exports.AgentExecutionDispatchError = exports.AGENT_EXECUTION_QUEUE_NAME = exports.AGENT_EXECUTION_JOB_TYPE = void 0;
exports.dispatchAgentExecutionPlan = dispatchAgentExecutionPlan;
exports.reconcileAgentExecutionDispatch = reconcileAgentExecutionDispatch;
exports.cancelAgentExecutionDispatch = cancelAgentExecutionDispatch;
exports.writeAgentExecutionCheckpoint = writeAgentExecutionCheckpoint;
exports.recordAgentExecutionBudgetUsage = recordAgentExecutionBudgetUsage;
exports.getAgentExecutionDispatchState = getAgentExecutionDispatchState;
const agent_persistence_1 = require("./agent-persistence");
const client_1 = require("./client");
exports.AGENT_EXECUTION_JOB_TYPE = 'agent.execution.plan';
exports.AGENT_EXECUTION_QUEUE_NAME = 'brovexa-work-v1';
const dependencyBlockedAtSql = `'infinity'::timestamptz`;
const budgetEffectPrefix = 'agent.execution.budget.';
class AgentExecutionDispatchError extends Error {
    code;
...
exports.AgentExecutionDispatchError = AgentExecutionDispatchError;
const terminalJobStatuses = new Set(['succeeded', 'failed', 'cancelled', 'review']);
const budgetFields = ['tokens', 'searches', 'apiCalls', 'credits', 'currencyMicros', 'runtimeMs'];
...
function assertIdentifier(value, field, maxLength = 128) {
    if (!isNonEmptyString(value) || value.length > maxLength) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', `${field} must be a non-empty identifier no longer than ${maxLength} characters.`);
    }
}
function assertAgentKey(value, field) {
    if (!/^agent\.[a-z0-9_.-]+$/.test(value)) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', `${field} must be a canonical agent key.`);
    }
}
function normalizeSupportedAgentKeys(values) {
    if (values.length < 1 || values.length > 128) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', 'supportedAgentKeys must contain 1 through 128 entries.');
    }
    const normalized = values.map((value) => value.trim());
    for (const value of normalized)
        assertAgentKey(value, 'supportedAgentKeys');
    if (new Set(normalized).size !== normalized.length) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', 'supportedAgentKeys must not contain duplicates.');
    }
    return normalized;
}
function parsePlan(row) {
    ...
    if (envelope.id !== row.id ||
        ...
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_PLAN_INVALID', `Execution plan ${row.id} does not match its canonical persistence projection.`);
    }
    const parsedSteps = [];
    ...
    for (const candidate of steps) {
        ...
        if (!key ||
            ...
            !budget ||
            stepKeys.has(key)) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_PLAN_INVALID', `Execution plan ${row.id} contains an invalid step projection.`);
        }
        stepKeys.add(key);
        parsedSteps.push({
            key,
    ...
    for (const step of parsedSteps) {
        for (const dependency of step.dependencies) {
            if (dependency === step.key || !stepKeys.has(dependency)) {
                throw new AgentExecutionDispatchError('AGENT_DISPATCH_PLAN_INVALID', `Execution plan ${row.id} contains an invalid dependency edge.`);
            }
        }
    }
    return {
...
async function assertCurrentWorkspaceRead(client, workspaceId, userId) {
    ...
    if (!result.rows[0]?.allowed) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_AUTHORIZATION_REQUIRED', 'Current active workspace.read authorization is required to dispatch an execution plan.');
    }
}
function resolveRetryLimit(row) {
    ...
    if (row.status !== 'approved' ||
        ...
        retryLimit < 0 ||
        retryLimit > 20) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_DEFINITION_NOT_APPROVED', `Specialist definition ${row.agent_key}@${row.version} is not dispatchable.`);
    }
    return { id: row.id, retryLimit };
}
function createWorkPayload(input, plan, step) {
    return {
        version: '1.0.0',
...
function jobIdempotencyKey(plan) {
    return `agent-plan:${plan.id}:v${plan.planVersion}`;
}
function workIdempotencyKey(plan, step) {
    return `${plan.id}:v${plan.planVersion}:${step.key}`;
}
async function findDispatchJob(client, plan) {
    const result = await client.query(`SELECT id, status
     FROM job_runs
     WHERE workspace_id = $1 AND job_type = $2 AND idempotency_key = $3`, [plan.workspaceId, exports.AGENT_EXECUTION_JOB_TYPE, jobIdempotencyKey(plan)]);
    return result.rows[0] ?? null;
}
async function assertExistingProjection(client, input, plan, jobRunId) {
    const work = await client.query(`SELECT
     ...
     ORDER BY payload->>'stepKey' ASC`, [jobRunId, plan.workspaceId]);
    if (work.rows.length !== plan.steps.length) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_PROJECTION_CONFLICT', `Existing JobRun for plan ${plan.id} has incomplete WorkUnit projection.`);
    }
    const expectedKeys = plan.steps.map((step) => step.key).sort();
    const actualKeys = work.rows.map((row) => row.step_key).sort();
    if (JSON.stringify(expectedKeys) !== JSON.stringify(actualKeys)) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_PROJECTION_CONFLICT', `Existing JobRun for plan ${plan.id} has a different step projection.`);
    }
    for (const row of work.rows) {
        if (row.dispatch_id !== input.dispatchId ||
            ...
            throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_DISPATCH_CONFLICT', `Execution plan ${plan.id} is already dispatched with different dispatcher identity or registry version.`);
        }
    }
}
async function assertDispatchIdUnused(client, workspaceId, dispatchId, planId) {
    const result = await client.query(`SELECT payload->>'planId' AS plan_id
     FROM job_work_units
     WHERE workspace_id = $1 AND payload->>'dispatchId' = $2
     LIMIT 1`, [workspaceId, dispatchId]);
    const existingPlanId = result.rows[0]?.plan_id;
    if (existingPlanId && existingPlanId !== planId) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_ID_CONFLICT', `Dispatch ID ${dispatchId} is already bound to plan ${existingPlanId}.`);
    }
}
async function unlockEligibleSteps(client, workspaceId, jobRunId, maxParallelism) {
    const active = await client.query(`SELECT count(*)::int AS count
     FROM job_work_units
    ...
    if (capacity === 0)
        return [];
    const eligible = await client.query(`SELECT candidate.id
     ...
     LIMIT $3
     FOR UPDATE OF candidate`, [workspaceId, jobRunId, capacity]);
...
async function settleBlockedAfterTerminalJob(client, workspaceId, jobRunId, jobStatus) {
    if (!['failed', 'cancelled', 'review'].includes(jobStatus))
        return;
    await client.query(`UPDATE job_work_units
     ...
     WHERE workspace_id = $1
       ...
       AND attempt_count = 0
       AND next_attempt_at = ${dependencyBlockedAtSql}`, [workspaceId, jobRunId]);
}
async function readDispatchState(client, workspaceId, dispatchId) {
    const jobs = await client.query(`SELECT DISTINCT
       work.job_run_id,
    ...
    if (jobs.rows.length !== 1 || !jobs.rows[0]) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_NOT_FOUND', `Execution dispatch ${dispatchId} was not found or has an invalid JobRun projection.`);
    }
    ...
    const work = await client.query(`SELECT
     ...
     GROUP BY work.id
     ORDER BY work.payload->>'stepKey' ASC`, [workspaceId, job.job_run_id, dispatchId]);
    return {
        dispatchId,
        planId: job.plan_id,
...
async function dispatchAgentExecutionPlan(pool, input) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        const planResult = await client.query(`SELECT
       ...
       FOR SHARE`, [input.planId, input.workspaceId]);
        const planRow = planResult.rows[0];
        if (!planRow) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_PLAN_NOT_FOUND', `Execution plan ${input.planId} was not found in workspace ${input.workspaceId}.`);
        }
        const plan = parsePlan(planRow);
        if (input.createdAt < plan.createdAt) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_PLAN_INVALID', 'Dispatch creation time cannot precede its execution plan.');
        }
        ...
        const orchestratorRun = await client.query(`SELECT status FROM agent_runs WHERE id = $1 AND workspace_id = $2 FOR SHARE`, [plan.runId, plan.workspaceId]);
        if (orchestratorRun.rows[0]?.status !== 'queued') {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_ORCHESTRATOR_RUN_STATE', `Orchestrator AgentRun ${plan.runId} must be queued before first dispatch.`);
        }
        const definitions = new Map();
        for (const step of plan.steps) {
            if (!supportedAgentKeys.has(step.agentKey)) {
                throw new AgentExecutionDispatchError('AGENT_DISPATCH_HANDLER_UNAVAILABLE', `No explicitly registered handler is available for ${step.agentKey}.`);
            }
            const definitionResult = await client.query(`SELECT id, agent_key, version, status, requires_human_approval, specification
         FROM agent_definitions
         WHERE agent_key = $1 AND version = $2
         FOR SHARE`, [step.agentKey, step.agentVersion]);
            const definitionRow = definitionResult.rows[0];
            if (!definitionRow) {
                throw new AgentExecutionDispatchError('AGENT_DISPATCH_DEFINITION_NOT_APPROVED', `Specialist definition ${step.agentKey}@${step.agentVersion} is unavailable.`);
            }
            definitions.set(step.key, resolveRetryLimit(definitionRow));
        }
        const job = await client.query(`INSERT INTO job_runs (
       ...
       RETURNING id, correlation_id`, [plan.workspaceId, exports.AGENT_EXECUTION_JOB_TYPE, jobIdempotencyKey(plan), input.createdAt]);
        const jobRow = job.rows[0];
        if (!jobRow) {
            const concurrent = await findDispatchJob(client, plan);
            if (!concurrent) {
                throw new AgentExecutionDispatchError('AGENT_DISPATCH_PROJECTION_CONFLICT', `Concurrent JobRun for plan ${plan.id} could not be resolved.`);
            }
        ...
        for (const step of plan.steps) {
            ...
            if (!definition)
                throw new Error(`Definition resolution missing for step ${step.key}.`);
            const payload = createWorkPayload(input, plan, step);
            await client.query(`INSERT INTO job_work_units (
           ...
           next_attempt_at, created_at, updated_at
                ...
                jobRow.id,
                plan.workspaceId,
                exports.AGENT_EXECUTION_QUEUE_NAME,
                step.agentKey,
        ...
        const newlyRunnableWorkUnitIds = await unlockEligibleSteps(client, plan.workspaceId, jobRow.id, plan.maxParallelism);
        const state = await readDispatchState(client, plan.workspaceId, input.dispatchId);
        return { ...state, created: true, newlyRunnableWorkUnitIds };
    });
}
async function reconcileAgentExecutionDispatch(pool, workspaceId, dispatchId) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        ...
        const newlyRunnableWorkUnitIds = await unlockEligibleSteps(client, workspaceId, stateBefore.jobRunId, stateBefore.maxParallelism);
        const state = await readDispatchState(client, workspaceId, dispatchId);
        return { ...state, created: false, newlyRunnableWorkUnitIds };
    });
}
async function cancelAgentExecutionDispatch(pool, workspaceId, dispatchId) {
    assertIdentifier(workspaceId, 'workspaceId');
    assertIdentifier(dispatchId, 'dispatchId');
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        const state = await readDispatchState(client, workspaceId, dispatchId);
       ...
       WHERE workspace_id = $1
         AND job_run_id = $2
         AND status NOT IN ('succeeded', 'cancelled', 'dead_letter', 'review')`, [workspaceId, state.jobRunId]);
        await client.query(`UPDATE job_runs
       SET status = 'cancelled', completed_at = COALESCE(completed_at, now()), updated_at = now()
       WHERE id = $1 AND workspace_id = $2 AND status NOT IN ('succeeded', 'failed', 'review')`, [state.jobRunId, workspaceId]);
        return readDispatchState(client, workspaceId, dispatchId);
    });
}
async function writeAgentExecutionCheckpoint(pool, input) {
    assertIdentifier(input.workspaceId, 'workspaceId');
    assertIdentifier(input.dispatchId, 'dispatchId');
    assertIdentifier(input.stepKey, 'stepKey');
    if (!/^[a-z][a-z0-9_.-]{0,127}$/.test(input.checkpointKey)) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', 'checkpointKey must use the canonical lowercase identifier format.');
    }
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
       ...
       FOR SHARE`, [input.workspaceId, input.dispatchId, input.stepKey]);
        const row = work.rows[0];
        if (!row) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_NOT_FOUND', `Step ${input.stepKey} was not found in dispatch ${input.dispatchId}.`);
        }
        if (row.is_blocked || (row.status !== 'running' && row.status !== 'retry_wait')) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_CHECKPOINT_STATE', `Step ${input.stepKey} must be running or retry_wait before writing a checkpoint.`);
        }
        const checkpoint = await client.query(`INSERT INTO job_checkpoints (work_unit_id, checkpoint_key, data)
       VALUES ($1, $2, $3::jsonb)
       ON CONFLICT (work_unit_id, checkpoint_key)
       DO UPDATE SET data = EXCLUDED.data, updated_at = now()
       RETURNING id`, [row.id, input.checkpointKey, JSON.stringify(input.data)]);
        const id = checkpoint.rows[0]?.id;
        if (!id)
            throw new Error('Agent execution checkpoint could not be persisted.');
        return id;
    });
}
function assertUsage(usage) {
    ...
    for (const field of budgetFields) {
        ...
        if (!Number.isSafeInteger(value) || value < 0) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', `usage.${field} must be a non-negative safe integer.`);
        }
        if (value > 0)
            nonzero = true;
    }
    if (!nonzero) {
        throw new AgentExecutionDispatchError('AGENT_DISPATCH_INPUT_INVALID', 'Budget usage must account for at least one non-zero resource.');
    }
}
async function recordAgentExecutionBudgetUsage(pool, input) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
       ...
       WHERE workspace_id = $1
         AND payload->>'dispatchId' = $2
         AND payload->>'stepKey' = $3
       FOR UPDATE`, [input.workspaceId, input.dispatchId, input.stepKey]);
        const row = work.rows[0];
        if (!row) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_NOT_FOUND', `Step ${input.stepKey} was not found in dispatch ${input.dispatchId}.`);
        }
        if (row.is_blocked || !['running', 'retry_wait', 'succeeded'].includes(row.status)) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_CHECKPOINT_STATE', `Step ${input.stepKey} cannot account usage while status is ${row.status}.`);
        }
        const reserved = readBudget(row.budget);
        if (!reserved) {
            throw new AgentExecutionDispatchError('AGENT_DISPATCH_PROJECTION_CONFLICT', `Step ${input.stepKey} has an invalid reserved budget projection.`);
        }
        ...
        const existing = await client.query(`SELECT data = $3::jsonb AS same_event
       FROM job_effects
       WHERE work_unit_id = $1 AND effect_key = $2`, [row.id, effectKey, JSON.stringify(data)]);
        if (existing.rows[0]) {
            if (!existing.rows[0].same_event) {
                throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_BUDGET_EVENT_ID_CONFLICT', `Budget event ${input.eventId} already exists with different content or scope.`);
            }
            return false;
        }
        const consumed = await client.query(`SELECT
       ...
       FROM job_effects
       WHERE work_unit_id = $1 AND data->>'kind' = 'agent_execution_budget_usage'`, [row.id]);
        const totals = consumed.rows[0];
        if (!totals)
            throw new Error('Budget usage aggregation returned no row.');
        const next = {
        ...
        for (const field of budgetFields) {
            if (!Number.isSafeInteger(next[field]) || next[field] > limits[field]) {
                throw new AgentExecutionDispatchError('AGENT_DISPATCH_BUDGET_EXCEEDED', `Step ${input.stepKey} exceeds reserved ${field} budget.`);
            }
        }
        await client.query(`INSERT INTO job_effects (work_unit_id, effect_key, data)
       VALUES ($1, $2, $3::jsonb)`, [row.id, effectKey, JSON.stringify(data)]);
        return true;
    });
}
async function getAgentExecutionDispatchState(pool, workspaceId, dispatchId) {
    assertIdentifier(workspaceId, 'workspaceId');
    assertIdentifier(dispatchId, 'dispatchId');
    const exists = await pool.query(`SELECT EXISTS (
       ...
       FROM job_work_units
       WHERE workspace_id = $1 AND payload->>'dispatchId' = $2
     ) AS exists`, [workspaceId, dispatchId]);
    if (!exists.rows[0]?.exists)
        return null;
    return (0, client_1.withPgTransaction)(pool, (client) => readDispatchState(client, workspaceId, dispatchId));
}
//# sourceMappingURL=agent-execution-dispatcher.js.map