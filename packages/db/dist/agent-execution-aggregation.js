...
exports.AgentExecutionAggregationError = void 0;
exports.aggregateAgentExecutionPlan = aggregateAgentExecutionPlan;
exports.getAgentExecutionAggregationState = getAgentExecutionAggregationState;
...
class AgentExecutionAggregationError extends Error {
    code;
...
exports.AgentExecutionAggregationError = AgentExecutionAggregationError;
...
function readUniqueStrings(value, maxItems) {
    if (!Array.isArray(value) || value.length > maxItems || value.some((entry) => !isNonEmptyString(entry)))
        return null;
...
function readStringRecord(value) {
    ...
    if (entries.some(([key, entry]) => !isNonEmptyString(key) || !isNonEmptyString(entry)))
        return null;
...
function assertIdentifier(value, field, maxLength = 128) {
    if (!value.trim() || value.length > maxLength) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_INPUT_INVALID', `${field} must be a non-empty identifier no longer than ${maxLength} characters.`);
    }
}
...
function parsePlanSteps(plan) {
    if (plan.envelope.id !== plan.id ||
        ...
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_PROJECTION_CONFLICT', `Execution plan ${plan.id} has an invalid canonical envelope projection.`);
    }
    const steps = [];
    const keys = new Set();
    for (const value of plan.envelope.steps) {
        ...
        if (!isNonEmptyString(value.key) ||
            ...
            !budget ||
            keys.has(value.key)) {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_PROJECTION_CONFLICT', `Execution plan ${plan.id} contains an invalid or duplicate step.`);
        }
        keys.add(value.key);
        steps.push({
            key: value.key,
...
function parseSpecialistResult(value) {
    if (!isRecord(value) || !isRecord(value.result) || value.validationState !== 'passed')
        return null;
    ...
    if ([inputTokens, outputTokens, searches, apiCalls, credits, currencyMicros].some((entry) => entry === null))
        return null;
    const toolSummary = [];
    ...
    for (const action of value.proposedActions) {
        if (!isRecord(action) || !isNonEmptyString(action.commandKey) || !isRecord(action.payload))
            return null;
...
function parseOrchestratorDefinition(row, run) {
    ...
    if (row.status !== 'approved' ||
        ...
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_DEFINITION_INVALID', `Orchestrator definition ${row.agent_key}@${row.version} is invalid for deterministic aggregation.`);
    }
    return {
        id: row.id,
        ...
        independentEvaluatorKey: typeof specification.independentEvaluatorKey === 'string' ? specification.independentEvaluatorKey : null,
        evalSuiteId: specification.evalSuiteId,
        evalThreshold,
    };
}
function parseEvaluatorDefinition(row, expectedKey) {
    const specification = row.specification;
    const modelPolicy = isRecord(specification.modelPolicy) ? specification.modelPolicy : null;
    ...
    if (row.status !== 'approved' ||
        ...
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_EVALUATOR_NOT_APPROVED', `Evaluator definition ${row.agent_key}@${row.version} is unavailable or invalid.`);
    }
    if (modelPolicy.routingMode !== 'deterministic_only' ||
        !Array.isArray(modelPolicy.allowedProviderIds) ||
        modelPolicy.allowedProviderIds.length !== 0 ||
        !Array.isArray(modelPolicy.allowedModelIds) ||
        modelPolicy.allowedModelIds.length !== 0 ||
        !Array.isArray(modelPolicy.fallbackModelIds) ||
        modelPolicy.fallbackModelIds.length !== 0) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_EVALUATOR_ROUTE_UNAVAILABLE', `Evaluator ${row.agent_key}@${row.version} requires a provider/model route that is not enabled in this slice.`);
    }
    return {
        id: row.id,
        agentKey: row.agent_key,
...
function deterministicId(prefix, ...parts) {
    return `${prefix}-${(0, node_crypto_1.createHash)('sha256').update(parts.join('\u0000')).digest('hex').slice(0, 48)}`;
}
function safeAdd(left, right) {
    const value = left + right;
    return Number.isSafeInteger(value) ? value : null;
}
function appendUnique(target, values, maxItems, issue, issues) {
    for (const value of values) {
        if (!target.includes(value))
            target.push(value);
    }
    target.sort();
    if (target.length > maxItems && !issues.includes(issue))
        issues.push(issue);
}
async function assertCurrentWorkspaceRead(client, workspaceId, userId) {
    ...
    if (!result.rows[0]?.allowed) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_AUTHORIZATION_REQUIRED', 'Current active workspace.read authorization is required to aggregate execution results.');
    }
}
function parseStoredAggregation(workspaceId, dispatchId, planId, planVersion, jobRunId, orchestratorRunId, run) {
    ...
    if (!isRecord(stored) || stored.dispatchId !== dispatchId || stored.planId !== planId || stored.planVersion !== planVersion) {
        throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EXECUTION_AGGREGATION_CONFLICT', `Orchestrator AgentRun ${orchestratorRunId} already contains a different execution aggregation.`);
    }
    if (!isNonEmptyString(stored.state) || !Array.isArray(stored.issues)) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_PROJECTION_CONFLICT', `Orchestrator AgentRun ${orchestratorRunId} contains an invalid aggregation projection.`);
    }
    ...
    const handoff = isRecord(stored.evaluatorHandoff) ? stored.evaluatorHandoff : null;
...
async function transitionRun(client, run, input) {
    if (run.status === input.toStatus || terminalRunStatuses.has(run.status)) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_ORCHESTRATOR_STATE', `Orchestrator AgentRun ${run.id} cannot transition from ${run.status} to ${input.toStatus}.`);
    }
    if (input.occurredAt < run.updated_at) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_INPUT_INVALID', `Aggregation transition time precedes orchestrator AgentRun ${run.id}.`);
    }
...
function stateForTerminalJob(jobStatus) {
    if (jobStatus === 'failed')
        return { state: 'failed', runStatus: 'failed' };
    if (jobStatus === 'cancelled')
        return { state: 'cancelled', runStatus: 'cancelled' };
    if (jobStatus === 'review')
        return { state: 'review_required', runStatus: 'review_required' };
    return null;
}
async function createEvaluatorHandoff(client, input, plan, orchestrator, evaluatorKey, aggregate) {
    if (!input.evaluatorAgentVersion) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_EVALUATOR_VERSION_REQUIRED', `Evaluator ${evaluatorKey} requires an exact evaluatorAgentVersion.`);
    }
    assertIdentifier(input.evaluatorAgentVersion, 'evaluatorAgentVersion', 64);
    const definitionResult = await client.query(`SELECT id, agent_key, version, status, autonomy_tier, requires_human_approval, specification
     FROM agent_definitions
     WHERE agent_key = $1 AND version = $2
     FOR SHARE`, [evaluatorKey, input.evaluatorAgentVersion]);
    const definitionRow = definitionResult.rows[0];
    if (!definitionRow) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_EVALUATOR_NOT_APPROVED', `Evaluator ${evaluatorKey}@${input.evaluatorAgentVersion} was not found.`);
    }
    const evaluator = parseEvaluatorDefinition(definitionRow, evaluatorKey);
    const parentContextResult = await client.query(`SELECT receipt, created_at
     FROM agent_context_receipts
     WHERE id = $1 AND workspace_id = $2
     FOR SHARE`, [plan.context_receipt_id, input.workspaceId]);
    const parentContext = parentContextResult.rows[0];
    if (!parentContext || input.occurredAt < parentContext.created_at) {
        throw new AgentExecutionAggregationError('AGENT_AGGREGATION_PROJECTION_CONFLICT', `Parent ContextReceipt ${plan.context_receipt_id} is unavailable or newer than the aggregation.`);
    }
    ...
    const contextInsert = await client.query(`INSERT INTO agent_context_receipts (
    ...
    if (!contextInsert.rows[0]) {
        const existing = await client.query(`SELECT workspace_id = $2::uuid
          AND user_id = $3::uuid
          AND run_scope_id = $4
       ...
       FROM agent_context_receipts WHERE id = $1`, [
        ...
        if (!existing.rows[0]?.same_context) {
            throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EVALUATOR_CONTEXT_ID_CONFLICT', 'Evaluator ContextReceipt identity conflict.');
        }
    }
    const runEnvelope = {
        id: evaluatorRunId,
    ...
    const runInsert = await client.query(`INSERT INTO agent_runs (
    ...
    if (!runInsert.rows[0]) {
       ...
       FROM agent_runs WHERE id = $1`, [
        ...
        if (!existing.rows[0]?.same_run) {
            throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_EVALUATOR_RUN_ID_CONFLICT', 'Evaluator AgentRun identity conflict.');
        }
    }
    return {
        handoffId,
...
async function aggregateAgentExecutionPlan(pool, input) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
       ...
       WHERE work.workspace_id = $1 AND work.payload->>'dispatchId' = $2`, [input.workspaceId, input.dispatchId]);
        ...
        if (jobs.rows.length !== 1 || !job || !job.plan_id || !job.orchestrator_run_id) {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_DISPATCH_NOT_FOUND', `Dispatch ${input.dispatchId} is missing or has an invalid JobRun projection.`);
        }
        const planResult = await client.query(`SELECT id, user_id, run_id, context_receipt_id, plan_version, step_count, envelope, created_at
       FROM agent_execution_plans
       WHERE id = $1 AND workspace_id = $2
       FOR SHARE`, [job.plan_id, input.workspaceId]);
        const plan = planResult.rows[0];
        if (!plan || plan.run_id !== job.orchestrator_run_id) {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_PROJECTION_CONFLICT', `Dispatch ${input.dispatchId} does not match its immutable execution plan.`);
        }
        if (input.occurredAt < plan.created_at) {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_INPUT_INVALID', 'Aggregation time cannot precede its execution plan.');
        }
        const steps = parsePlanSteps(plan);
        await assertCurrentWorkspaceRead(client, input.workspaceId, plan.user_id);
        const runResult = await client.query(`SELECT id, agent_definition_id, agent_key, agent_version, status, envelope,
              started_at, completed_at, updated_at
       FROM agent_runs
       WHERE id = $1 AND workspace_id = $2
       FOR UPDATE`, [plan.run_id, input.workspaceId]);
        let orchestratorRun = runResult.rows[0];
        if (!orchestratorRun) {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_PROJECTION_CONFLICT', `Orchestrator AgentRun ${plan.run_id} is missing.`);
        }
        const stored = parseStoredAggregation(input.workspaceId, input.dispatchId, plan.id, plan.plan_version, job.job_run_id, plan.run_id, orchestratorRun);
        if (stored)
            return stored;
        if (orchestratorRun.status !== 'queued') {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_ORCHESTRATOR_STATE', `Orchestrator AgentRun ${plan.run_id} is ${orchestratorRun.status} without an aggregation projection.`);
        }
        ...
        if (job.job_status !== 'succeeded') {
            throw new AgentExecutionAggregationError('AGENT_AGGREGATION_NOT_READY', `Dispatch ${input.dispatchId} JobRun is ${job.job_status}; completed-DAG aggregation requires a terminal JobRun.`);
        }
        const definitionResult = await client.query(`SELECT id, agent_key, version, status, autonomy_tier, requires_human_approval, specification
       FROM agent_definitions
       WHERE id = $1 AND agent_key = $2 AND version = $3
       FOR SHARE`, [orchestratorRun.agent_definition_id, orchestratorRun.agent_key, orchestratorRun.agent_version]);
        const definitionRow = definitionResult.rows[0];
        if (!definitionRow) {
       ...
       ORDER BY work.payload->>'stepKey' ASC`, [input.workspaceId, job.job_run_id, resultEffectKey, input.dispatchId]);
        const issues = [];
        if (workResult.rows.length !== steps.length)
            issues.push('plan_work_count_mismatch');
        for (const validator of orchestrator.deterministicValidators) {
            if (!supportedOrchestratorValidators.has(validator))
                issues.push(`unsupported_validator:${validator}`);
        }
        ...
        for (const work of workResult.rows) {
            ...
            if (payload.dispatchId !== input.dispatchId ||
            ...
            const childResult = await client.query(`SELECT id, status, parent_run_id, handoff_id, context_receipt_id, agent_key, agent_version, envelope
         FROM agent_runs
         WHERE id = $1 AND workspace_id = $2
         FOR SHARE`, [effect.runId, input.workspaceId]);
            ...
            if (!child ||
                ...
                !(0, node_util_1.isDeepStrictEqual)(projectedResult, effect.result)) {
            ...
            for (const [field, limit] of [
                ...
                if (next === null)
                    issues.push(`budget_overflow:${field}`);
                else
                    resourceUsage[field] = next;
            }
            ...
            for (const action of result.proposedActions) {
                if (!step.commandKeys.includes(action.commandKey))
                    issues.push(`action_outside_plan:${stepKey}:${action.commandKey}`);
            }
            ...
            if (toolSummary.length > 512)
                issues.push('aggregate_tool_summary_limit');
            if (proposedActions.length > 128)
                issues.push('aggregate_proposed_action_limit');
            aggregateSteps.push({
                stepKey: step.key,
                workUnitId: work.id,
        ...
        if (aggregateSteps.length !== steps.length)
            issues.push('aggregate_step_count_mismatch');
        if (orchestrator.evidenceRequired && evidenceIds.length === 0)
            issues.push('orchestrator_evidence_required');
        if (aggregateConfidence < orchestrator.minimumConfidence ||
        ...
        if (orchestrator.independentEvaluatorKey) {
            const handoff = await createEvaluatorHandoff(client, input, plan, orchestrator, orchestrator.independentEvaluatorKey, aggregate);
            const storedAggregation = {
                version: '1.0.0',
                dispatchId: input.dispatchId,
                planId: plan.id,
...
async function getAgentExecutionAggregationState(pool, workspaceId, dispatchId) {
    ...
    if (result.rows.length !== 1 || !row)
        return null;
    return parseStoredAggregation(workspaceId, dispatchId, row.plan_id, row.plan_version, row.job_run_id, row.orchestrator_run_id, {
...
//# sourceMappingURL=agent-execution-aggregation.js.map