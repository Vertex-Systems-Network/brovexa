...
exports.AgentSpecialistExecutionError = void 0;
exports.createAgentSpecialistRunId = createAgentSpecialistRunId;
exports.createAgentSpecialistContextReceiptId = createAgentSpecialistContextReceiptId;
exports.createAgentSpecialistTransitionId = createAgentSpecialistTransitionId;
exports.prepareAgentSpecialistAttempt = prepareAgentSpecialistAttempt;
exports.completeAgentSpecialistAttempt = completeAgentSpecialistAttempt;
exports.getAgentSpecialistRunsForWorkUnit = getAgentSpecialistRunsForWorkUnit;
const node_crypto_1 = require("node:crypto");
const node_util_1 = require("node:util");
const agent_persistence_1 = require("./agent-persistence");
const client_1 = require("./client");
class AgentSpecialistExecutionError extends Error {
    code;
...
exports.AgentSpecialistExecutionError = AgentSpecialistExecutionError;
...
function readIdentifier(value, field, maxLength = 128) {
    if (!isNonEmptyString(value) || value.length > maxLength) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_INPUT_INVALID', `${field} must be a non-empty identifier no longer than ${maxLength} characters.`);
    }
    return value.trim();
}
function readStringArray(value, field, maxItems = safeArrayMaximum) {
    if (!Array.isArray(value) || value.length > maxItems || value.some((item) => !isNonEmptyString(item))) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_INPUT_INVALID', `${field} must be a bounded array of non-empty strings.`);
    }
    const normalized = value.map((item) => item.trim());
    if (new Set(normalized).size !== normalized.length) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_INPUT_INVALID', `${field} must not contain duplicate identifiers.`);
    }
    return normalized;
}
...
function readBudget(value, field) {
    if (!isRecord(value)) {
...
function parseWorkPayload(value) {
    if (!isRecord(value)) {
    ...
    if (budget.maxConcurrency !== 1) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_INPUT_INVALID', 'Specialist WorkUnit must reserve exactly one concurrency slot.');
    }
    return {
        version: '1.0.0',
...
function parseDefinition(row, payload) {
    if (row.status !== 'approved' || row.requires_human_approval) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_DEFINITION_NOT_APPROVED', `Specialist definition ${payload.agentKey}@${payload.agentVersion} is not execution-approved.`);
    }
    ...
    if (specification.key !== row.agent_key ||
        ...
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_DEFINITION_NOT_APPROVED', `Specialist definition ${payload.agentKey}@${payload.agentVersion} is internally inconsistent.`);
    }
    ...
    if (!isRecord(modelPolicy) ||
        ...
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_DETERMINISTIC_ONLY', `Specialist ${payload.agentKey}@${payload.agentVersion} is not deterministic-only.`);
    }
    if (isNonEmptyString(specification.independentEvaluatorKey)) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_EVALUATOR_REQUIRED', `Specialist ${payload.agentKey}@${payload.agentVersion} requires independent evaluation not implemented in this slice.`);
    }
    ...
    for (const commandKey of payload.commandKeys) {
        if (!allowedCommands.includes(commandKey)) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_SCOPE_MISMATCH', `Command ${commandKey} is outside the specialist definition.`);
        }
    }
    ...
    for (const field of [
        ...
        if (payload.budget[field] > definitionBudget[field]) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_SCOPE_MISMATCH', `Specialist WorkUnit exceeds definition budget ${field}.`);
        }
    }
    ...
    if (!Number.isFinite(minimumConfidence) ||
        ...
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_DEFINITION_NOT_APPROVED', 'Specialist confidence thresholds are invalid.');
    }
    return {
        id: row.id,
...
function assertCurrentWorkspaceReadResult(allowed) {
    if (!allowed) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_AUTHORIZATION_REQUIRED', 'Current active workspace.read authorization is required for specialist execution.');
    }
}
function hashIdentity(...parts) {
    return (0, node_crypto_1.createHash)('sha256').update(parts.join('\u0000')).digest('hex').slice(0, 48);
}
function createAgentSpecialistRunId(workUnitId, attempt) {
    return `agent-specialist-${hashIdentity(workUnitId, attempt)}`;
}
function createAgentSpecialistContextReceiptId(workUnitId, attempt) {
    return `ctx-specialist-${hashIdentity(workUnitId, attempt)}`;
}
function createAgentSpecialistTransitionId(runId, phase) {
    return `agent-specialist-${phase}-${hashIdentity(runId, phase)}`;
}
async function resolveAdmission(pool, input) {
    ...
    const workResult = await pool.query(`SELECT
     ...
     WHERE work.id = $1 AND work.workspace_id = $2`, [input.workUnitId, input.workspaceId]);
    ...
    if (work.status !== 'running' ||
        ...
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_WORK_STATE', `WorkUnit ${input.workUnitId} is not the claimed running attempt ${input.attempt}.`);
    }
    if (work.job_type !== canonicalJobType ||
        ...
        !(0, node_util_1.isDeepStrictEqual)(work.payload, input.payload)) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_WORK_IDENTITY_MISMATCH', `WorkUnit ${input.workUnitId} does not match its claimed canonical specialist identity.`);
    }
    const planResult = await pool.query(`SELECT user_id, run_id, context_receipt_id, plan_version, max_parallelism, envelope
     FROM agent_execution_plans
     WHERE id = $1 AND workspace_id = $2`, [payload.planId, input.workspaceId]);
    ...
    const step = plan.envelope.steps.find((candidate) => isRecord(candidate) && candidate.key === payload.stepKey);
    if (!step) {
    ...
    if (plan.run_id !== payload.orchestratorRunId ||
        ...
        !(0, node_util_1.isDeepStrictEqual)(step, expectedStep)) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_PLAN_MISMATCH', `WorkUnit ${input.workUnitId} no longer matches immutable plan ${payload.planId}.`);
    }
    const authorization = await pool.query(`SELECT EXISTS (
       SELECT 1
     ...
     ) AS allowed`, [input.workspaceId, plan.user_id]);
    assertCurrentWorkspaceReadResult(authorization.rows[0]?.allowed ?? false);
    const definitionResult = await pool.query(`SELECT id, agent_key, version, status, requires_human_approval, specification
     FROM agent_definitions
     WHERE agent_key = $1 AND version = $2`, [payload.agentKey, payload.agentVersion]);
    const definitionRow = definitionResult.rows[0];
    if (!definitionRow) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_DEFINITION_NOT_APPROVED', `Specialist definition ${payload.agentKey}@${payload.agentVersion} is unavailable.`);
    }
    const definition = parseDefinition(definitionRow, payload);
    const contextResult = await pool.query(`SELECT receipt, created_at
     FROM agent_context_receipts
     WHERE id = $1 AND workspace_id = $2`, [payload.contextReceiptId, input.workspaceId]);
    const parentContext = contextResult.rows[0];
    if (!parentContext || !isRecord(parentContext.receipt)) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_CONTEXT_MISMATCH', `Parent ContextReceipt ${payload.contextReceiptId} is unavailable.`);
    }
    return { work, payload, userId: plan.user_id, definition, parentContext };
}
function selectChildMemoryRefs(parentReceipt, payload) {
    ...
    for (const value of parentReceipt.memoryRefs) {
        if (isRecord(value) && isNonEmptyString(value.memoryId))
            byId.set(value.memoryId, value);
    }
    const selected = [];
    for (const memoryId of payload.memoryRefs) {
        const memory = byId.get(memoryId);
        if (!memory || memory.status !== 'active') {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_CONTEXT_MISMATCH', `Specialist memory ${memoryId} is outside the active parent ContextReceipt.`);
        }
        selected.push({ ...memory });
    }
    return selected;
}
function createQueuedRunEnvelope(runId, contextReceiptId, workUnitId, payload, definition) {
    return {
        id: runId,
...
async function prepareAgentSpecialistAttempt(pool, input) {
    const admission = await resolveAdmission(pool, input);
    const { work, payload, definition, parentContext, userId } = admission;
    const previous = await pool.query(`SELECT id, status, updated_at, envelope
     FROM agent_runs
     WHERE workspace_id = $1
       ...
       AND agent_key = $4
       AND agent_version = $5
     ORDER BY created_at ASC, id ASC`, [payload.workspaceId, work.id, payload.orchestratorRunId, payload.agentKey, payload.agentVersion]);
    for (const row of previous.rows) {
        if (row.status === 'succeeded') {
            ...
            if (!isResultEnvelope(projected)) {
                throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_RUN_STATE', `Succeeded specialist AgentRun ${row.id} has an invalid result projection.`);
            }
            return {
                payload,
                userId,
        ...
        if (['review_required', 'blocked', 'budget_stopped'].includes(row.status)) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_PRIOR_REVIEW_REQUIRED', `Prior specialist AgentRun ${row.id} requires review before another attempt.`);
        }
    }
    ...
    if (!existingCurrent) {
        await (0, agent_persistence_1.persistAgentRun)(pool, {
            id: runId,
            workspaceId: payload.workspaceId,
            agentDefinitionId: definition.id,
            agentKey: payload.agentKey,
    ...
    const current = await pool.query(`SELECT status, updated_at
     FROM agent_runs
     WHERE id = $1 AND workspace_id = $2`, [runId, payload.workspaceId]);
    const currentRow = current.rows[0];
    if (!currentRow || (currentRow.status !== 'queued' && currentRow.status !== 'running')) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_RUN_STATE', `Specialist AgentRun ${runId} is not resumable for attempt ${input.attempt}.`);
    }
    const abandonedRuns = previous.rows
        .filter((row) => row.id !== runId && (row.status === 'queued' || row.status === 'running'))
        .map((row) => ({ runId: row.id, status: row.status, updatedAt: row.updated_at }));
    return {
        payload,
        userId,
        definition,
...
function assertResultStringList(value, field, maxItems) {
...
function normalizeResult(value, payload, definition) {
    if (!isRecord(value) || !isRecord(value.result)) {
    ...
    if (value.validationState !== 'passed') {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_REVIEW_REQUIRED', 'Specialist result cannot complete while deterministic validation has not passed.');
    }
    if (confidence < definition.minimumConfidence || confidence < definition.reviewBelowConfidence) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_REVIEW_REQUIRED', `Specialist confidence ${confidence} is below the configured completion threshold.`);
    }
    ...
    if (definition.evidenceRequired && evidenceIds.length === 0) {
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_REVIEW_REQUIRED', 'Specialist definition requires evidence before successful completion.');
    }
    ...
    const toolSummary = value.toolSummary.map((entry, index) => {
        if (!isRecord(entry)) {
        ...
        if (!payload.toolKeys.includes(toolKey)) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_SCOPE_MISMATCH', `Specialist result claims unapproved tool ${toolKey}.`);
        }
    ...
    const proposedActions = value.proposedActions.map((entry, index) => {
        ...
        if (!payload.commandKeys.includes(commandKey)) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_SCOPE_MISMATCH', `Specialist result proposes unapproved command ${commandKey}.`);
        }
        return {
            commandKey,
    ...
    if (!Number.isSafeInteger(tokenTotal) ||
        ...
        throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_SCOPE_MISMATCH', 'Specialist result cost exceeds its reserved WorkUnit budget.');
    }
    return {
        result: value.result,
        confidence,
...
async function completeAgentSpecialistAttempt(pool, input) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        const current = await client.query(`SELECT status, last_transition_id, envelope, started_at, completed_at, updated_at
       FROM agent_runs
       ...
       FOR UPDATE`, [input.workspaceId, input.runId]);
        ...
        if (run.status === 'succeeded') {
            const same = run.last_transition_id === transitionId &&
                (0, node_util_1.isDeepStrictEqual)(run.envelope.result, normalized.result) &&
                run.envelope.confidence === normalized.confidence &&
                (0, node_util_1.isDeepStrictEqual)(run.envelope.evidenceIds, normalized.evidenceIds) &&
                (0, node_util_1.isDeepStrictEqual)(run.envelope.toolSummary, normalized.toolSummary) &&
                (0, node_util_1.isDeepStrictEqual)(run.envelope.cost, normalized.cost) &&
                (0, node_util_1.isDeepStrictEqual)(run.envelope.proposedActions, normalized.proposedActions);
            if (!same) {
                throw new agent_persistence_1.AgentPersistenceConflictError('AGENT_SPECIALIST_RESULT_CONFLICT', `Specialist AgentRun ${input.runId} is already succeeded with a different result projection.`);
            }
            return { created: false, result: normalized };
        }
        if (run.status !== 'running' || !run.started_at) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_RUN_STATE', `Specialist AgentRun ${input.runId} must be running before completion.`);
        }
        if (input.occurredAt < run.updated_at) {
            throw new AgentSpecialistExecutionError('AGENT_SPECIALIST_RUN_STATE', `Specialist AgentRun ${input.runId} completion time regresses its lifecycle projection.`);
        }
       ...
       SET status = 'succeeded',
           ...
           completed_at = $5,
           updated_at = $5
       WHERE workspace_id = $1 AND id = $2`, [input.workspaceId, input.runId, transitionId, JSON.stringify(finalProjection), input.occurredAt]);
        return { created: true, result: normalized };
    });
}
async function getAgentSpecialistRunsForWorkUnit(pool, workspaceId, workUnitId) {
    const result = await pool.query(`SELECT id, status, parent_run_id, context_receipt_id, envelope
     FROM agent_runs
     WHERE workspace_id = $1 AND handoff_id = $2
     ORDER BY created_at ASC, id ASC`, [workspaceId, workUnitId]);
...
//# sourceMappingURL=agent-specialist-execution.js.map