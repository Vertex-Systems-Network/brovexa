...
exports.SourcePaginationCoverageChainSchema = void 0;
exports.parseSourcePaginationCoverageChain = parseSourcePaginationCoverageChain;
...
const SourcePaginationCoveragePageSchema = zod_1.z
    .object({
    pageIndex: zod_1.z.number().int().min(0).max(9_999),
    request: source_1.SourceRequestEnvelopeSchema,
    admission: source_1.ConnectorAdmissionDecisionSchema,
    result: source_1.SourceResultEnvelopeSchema,
})
    .strict();
const SourcePaginationContinuationSchema = zod_1.z
    .object({
    hasMore: zod_1.z.boolean(),
    nextCursor: zod_1.z.string().max(4096).optional(),
    nextPage: zod_1.z.number().int().positive().optional(),
})
    .strict();
const SourcePaginationCumulativeUsageSchema = zod_1.z
    .object({
    ...
    .strict();
const SourcePaginationAggregateCoverageSchema = zod_1.z
    .object({
...
exports.SourcePaginationCoverageChainSchema = zod_1.z
    .object({
    version: zod_1.z.literal('1.0.0'),
    chainId: IdentifierSchema,
    mode: zod_1.z.enum(['cursor', 'page']),
    ...
    policySnapshot: zod_1.z.object({ policyId: IdentifierSchema, policyVersion: VersionSchema }).strict(),
    budgetSnapshot: source_1.SourceBudgetSchema,
    pages: zod_1.z.array(SourcePaginationCoveragePageSchema).min(1).max(10_000),
    ...
    const returnedRecordValues = [];
    const requestUsageValues = [];
    const pageUsageValues = [];
    const byteUsageValues = [];
    const currencyUsageValues = [];
    const runtimeUsageValues = [];
    for (let index = 0; index < chain.pages.length; index += 1) {
        ...
        if (request.workspaceId !== chain.workspaceId ||
        ...
        if (admission.connectorKey !== request.connectorKey ||
    ...
    if (chain.cumulativeUsage.requests !== sums.requests ||
    ...
    if ((sums.requests ?? 0) > chain.budgetSnapshot.maxRequests ||
...
function parseSourcePaginationCoverageChain(rawChain) {
    return exports.SourcePaginationCoverageChainSchema.parse(rawChain);
}
//# sourceMappingURL=source-pagination-coverage.js.map