...
exports.classifySourceIpAddress = classifySourceIpAddress;
exports.isPublicSourceIpAddress = isPublicSourceIpAddress;
...
function embeddedIpv4FromIpv6(value) {
...
function classifyIpv6(address) {
    ...
    if (value === 1n)
        return 'loopback';
    if (value === ipv6ToBigInt('fd00:ec2::254'))
        return 'metadata';
    if (ipv6InCidr(value, '::ffff:0:0', 96)) {
        return classifyIpv4(embeddedIpv4FromIpv6(value));
    }
    if (ipv6InCidr(value, 'fc00::', 7))
        return 'private';
    if (ipv6InCidr(value, 'fe80::', 10))
        return 'link_local';
    if (ipv6InCidr(value, 'ff00::', 8))
        return 'multicast';
    if (ipv6InCidr(value, '2001:db8::', 32))
        return 'documentation';
    if (ipv6InCidr(value, '100::', 64) ||
...
function classifySourceIpAddress(rawAddress) {
    const address = rawAddress.trim().toLowerCase();
    if (address.includes('%'))
        return { address, family: 0, classification: 'invalid' };
    const family = (0, node_net_1.isIP)(address);
    if (family === 4)
        return { address, family: 4, classification: classifyIpv4(address) };
    if (family === 6)
        return { address, family: 6, classification: classifyIpv6(address) };
    return { address, family: 0, classification: 'invalid' };
}
function isPublicSourceIpAddress(address) {
    return classifySourceIpAddress(address).classification === 'public';
}
//# sourceMappingURL=source-ip-classifier.js.map