...
exports.SourceDiscoveryCheckpointPersistenceError = void 0;
exports.saveSourceDiscoveryCheckpoint = saveSourceDiscoveryCheckpoint;
exports.getSourceDiscoveryCheckpoint = getSourceDiscoveryCheckpoint;
exports.requireSourceDiscoveryCheckpoint = requireSourceDiscoveryCheckpoint;
const identifierPattern = /^[A-Za-z0-9][A-Za-z0-9_.:-]{0,255}$/;
const safeIntegerMax = Number.MAX_SAFE_INTEGER;
class SourceDiscoveryCheckpointPersistenceError extends Error {
    code;
...
exports.SourceDiscoveryCheckpointPersistenceError = SourceDiscoveryCheckpointPersistenceError;
...
function assertSafeInteger(value, field, positive = false) {
...
function validateInput(input) {
    ...
    if (typeof input.workspaceId !== 'string' || input.workspaceId.length === 0)
        throw inputError('workspaceId is required.');
    if (input.mode !== 'cursor' && input.mode !== 'page')
        throw inputError('mode must be cursor or page.');
    ...
    if (input.nextPage !== null)
        assertSafeInteger(input.nextPage, 'nextPage', true);
    if (input.mode === 'cursor' && input.nextPage !== null)
        throw inputError('cursor checkpoints cannot declare nextPage.');
    if (input.mode === 'page' && input.nextCursor !== null)
        throw inputError('page checkpoints cannot declare nextCursor.');
    ...
    for (const [field, value] of Object.entries(input.cumulativeUsage))
        assertSafeInteger(value, `cumulativeUsage.${field}`);
    if (!['complete', 'partial', 'unknown'].includes(input.coverageState))
        throw inputError('coverageState is invalid.');
    ...
    if (!(input.observedAt instanceof Date) || Number.isNaN(input.observedAt.getTime()))
        throw inputError('observedAt must be a valid Date.');
    if (input.expectedVersion !== null)
        assertSafeInteger(input.expectedVersion, 'expectedVersion', true);
}
function readSafeInteger(value, field) {
    const parsed = Number(value);
    if (!Number.isSafeInteger(parsed) || parsed < 0) {
        throw new SourceDiscoveryCheckpointPersistenceError('SOURCE_DISCOVERY_CHECKPOINT_INPUT_INVALID', `${field} is outside the JavaScript safe integer range.`);
    }
    return parsed;
}
...
async function saveSourceDiscoveryCheckpoint(pool, input) {
    validateInput(input);
    const values = [
        input.id,
        input.workspaceId,
    ...
    if (input.expectedVersion === null) {
        const inserted = await pool.query(`INSERT INTO source_discovery_checkpoints (
       ...
       RETURNING ${returningColumns}`, values);
        const row = inserted.rows[0];
        if (row)
            return rowToCheckpoint(row);
    }
    else {
        const updated = await pool.query(`UPDATE source_discovery_checkpoints
       ...
       WHERE id = $1
         ...
         AND observed_at <= $16
       RETURNING ${returningColumns}`, [...values, input.expectedVersion]);
        const row = updated.rows[0];
        if (row)
            return rowToCheckpoint(row);
    }
    throw new SourceDiscoveryCheckpointPersistenceError('SOURCE_DISCOVERY_CHECKPOINT_CONFLICT', `Checkpoint for source task ${input.sourceTaskId} changed, regressed or already exists; reload before retrying.`);
}
async function getSourceDiscoveryCheckpoint(pool, input) {
    if (typeof input.workspaceId !== 'string' || input.workspaceId.length === 0)
        throw inputError('workspaceId is required.');
    assertIdentifier(input.sourceTaskId, 'sourceTaskId');
    const result = await pool.query(`SELECT ${returningColumns}
     FROM source_discovery_checkpoints
     WHERE workspace_id = $1::uuid AND source_task_id = $2`, [input.workspaceId, input.sourceTaskId]);
    const row = result.rows[0];
    return row ? rowToCheckpoint(row) : null;
}
async function requireSourceDiscoveryCheckpoint(pool, input) {
    const checkpoint = await getSourceDiscoveryCheckpoint(pool, input);
    if (!checkpoint) {
        throw new SourceDiscoveryCheckpointPersistenceError('SOURCE_DISCOVERY_CHECKPOINT_NOT_FOUND', `No discovery checkpoint exists for source task ${input.sourceTaskId}.`);
    }
    return checkpoint;
}
//# sourceMappingURL=source-discovery-checkpoint-persistence.js.map