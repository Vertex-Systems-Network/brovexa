...
exports.SourceTaskPersistenceError = exports.SOURCE_EXECUTION_RESULT_EFFECT = exports.SOURCE_EXECUTION_QUEUE_NAME = exports.SOURCE_EXECUTION_WORK_TYPE = exports.SOURCE_EXECUTION_JOB_TYPE = void 0;
exports.persistResearchJobPreflight = persistResearchJobPreflight;
exports.getResearchJobPreflight = getResearchJobPreflight;
exports.getSourceTaskState = getSourceTaskState;
exports.createSourceTask = createSourceTask;
exports.claimSourceTask = claimSourceTask;
exports.recordSourceTaskUsage = recordSourceTaskUsage;
exports.recordSourceTaskFailure = recordSourceTaskFailure;
exports.cancelSourceTask = cancelSourceTask;
exports.completeSourceTask = completeSourceTask;
exports.getResearchJobPreflightState = getResearchJobPreflightState;
const client_1 = require("./client");
const jobs_1 = require("./jobs");
const source_registry_persistence_1 = require("./source-registry-persistence");
exports.SOURCE_EXECUTION_JOB_TYPE = 'source.execute';
exports.SOURCE_EXECUTION_WORK_TYPE = 'source.execute';
exports.SOURCE_EXECUTION_QUEUE_NAME = 'brovexa-work-v1';
exports.SOURCE_EXECUTION_RESULT_EFFECT = 'source.execution.result';
...
class SourceTaskPersistenceError extends Error {
    code;
...
exports.SourceTaskPersistenceError = SourceTaskPersistenceError;
...
function assertDate(value, field, code) {
    if (!(value instanceof Date) || Number.isNaN(value.getTime())) {
        throw new SourceTaskPersistenceError(code, `${field} must be a valid Date.`);
    }
}
function readSafeInteger(value, field, positive = false) {
...
function addSafe(left, right, field) {
...
function assertUniqueIdentifiers(values, field, code) {
...
function readAdmissionBudget(snapshot) {
    return parseBudget(snapshot.admission.effectiveBudget);
}
function readResearchJobId(snapshot) {
    const value = snapshot.request.researchJobId;
    return typeof value === 'string' && value.length > 0 ? value : null;
}
function readExecutionIntent(snapshot) {
...
async function persistResearchJobPreflight(pool, input) {
    ...
    for (const snapshotId of snapshotIds) {
        const snapshot = await (0, source_registry_persistence_1.getSourceAdmissionSnapshot)(pool, input.workspaceId, snapshotId);
        if (!snapshot) {
            throw new SourceTaskPersistenceError('SOURCE_PREFLIGHT_SNAPSHOT_NOT_FOUND', `Admission snapshot ${snapshotId} is not available in workspace ${input.workspaceId}.`);
        }
        if (readResearchJobId(snapshot) !== input.researchJobId || readExecutionIntent(snapshot) !== 'execute') {
            throw new SourceTaskPersistenceError('SOURCE_PREFLIGHT_SNAPSHOT_IDENTITY_MISMATCH', `Admission snapshot ${snapshotId} is not an execute admission for research job ${input.researchJobId}.`);
        }
        snapshots.push(snapshot);
    }
    const aggregateBudget = snapshots.reduce((sum, snapshot) => addBudget(sum, readAdmissionBudget(snapshot)), emptyBudget());
    const decision = snapshots.reduce((current, snapshot) => (decisionRank(snapshot.decision) > decisionRank(current) ? snapshot.decision : current), 'allow');
    const envelope = {
        id: input.id,
    ...
    const inserted = await pool.query(`INSERT INTO research_job_preflights (
    ...
    if (inserted.rows[0]?.id)
        return { id: input.id, created: true, envelope };
    const byId = await pool.query('SELECT envelope FROM research_job_preflights WHERE id = $1', [input.id]);
    if (byId.rows[0]) {
    ...
    const byIdempotency = await pool.query(`SELECT id, envelope FROM research_job_preflights
     WHERE workspace_id = $1 AND research_job_id = $2 AND idempotency_key = $3`, [input.workspaceId, input.researchJobId, input.idempotencyKey]);
    const existing = byIdempotency.rows[0];
    if (existing) {
        throw new SourceTaskPersistenceError('SOURCE_PREFLIGHT_IDEMPOTENCY_CONFLICT', `Research job ${input.researchJobId} idempotency key ${input.idempotencyKey} is already bound to preflight ${existing.id}.`);
    }
    throw new Error('Research job preflight insert did not return a row or resolvable conflict.');
}
async function getResearchJobPreflight(pool, workspaceId, preflightId) {
    ...
    const result = await pool.query('SELECT envelope FROM research_job_preflights WHERE workspace_id = $1 AND id = $2', [workspaceId, preflightId]);
    return result.rows[0]?.envelope ?? null;
}
async function createCanonicalSourceWork(client, input) {
    const insertedRun = await client.query(`INSERT INTO job_runs (workspace_id, job_type, job_version, idempotency_key)
     VALUES ($1, $2, 1, $3)
     ON CONFLICT (workspace_id, job_type, idempotency_key) DO NOTHING
     RETURNING id, correlation_id`, [input.workspaceId, exports.SOURCE_EXECUTION_JOB_TYPE, input.sourceTaskId]);
    let jobRunId = insertedRun.rows[0]?.id;
    let correlationId = insertedRun.rows[0]?.correlation_id;
    if (!jobRunId || !correlationId) {
        const existingRun = await client.query(`SELECT id, correlation_id FROM job_runs
       WHERE workspace_id = $1 AND job_type = $2 AND idempotency_key = $3
       FOR UPDATE`, [input.workspaceId, exports.SOURCE_EXECUTION_JOB_TYPE, input.sourceTaskId]);
        jobRunId = existingRun.rows[0]?.id;
        correlationId = existingRun.rows[0]?.correlation_id;
    }
    if (!jobRunId || !correlationId)
        throw new Error('Canonical source job identity could not be resolved.');
    const insertedWork = await client.query(`INSERT INTO job_work_units (
     ...
     RETURNING id`, [
        jobRunId,
        input.workspaceId,
        exports.SOURCE_EXECUTION_QUEUE_NAME,
        exports.SOURCE_EXECUTION_WORK_TYPE,
        input.sourceTaskId,
        correlationId,
    ...
    if (!workUnitId) {
        const existingWork = await client.query(`SELECT id, payload, max_attempts FROM job_work_units
       WHERE job_run_id = $1 AND work_type = $2 AND idempotency_key = $3
       FOR UPDATE`, [jobRunId, exports.SOURCE_EXECUTION_WORK_TYPE, input.sourceTaskId]);
        const row = existingWork.rows[0];
        if (!row)
            throw new Error('Canonical source work identity could not be resolved.');
...
async function loadSourceTaskRow(pool, workspaceId, sourceTaskId) {
    const result = await pool.query(`SELECT
     ...
     LIMIT 1`, [workspaceId, sourceTaskId]);
    return result.rows[0] ?? null;
}
async function readConsumedUsage(pool, workspaceId, sourceTaskId) {
    const result = await pool.query(`SELECT
     ...
     WHERE workspace_id = $1 AND source_task_id = $2`, [workspaceId, sourceTaskId]);
...
function toSourceTaskState(row, consumed) {
    return {
        sourceTaskId: row.id,
...
async function getSourceTaskState(pool, workspaceId, sourceTaskId) {
...
async function createSourceTask(pool, input) {
    ...
    const created = await (0, client_1.withPgTransaction)(pool, async (client) => {
        const preflightResult = await client.query(`SELECT research_job_id, decision, admission_snapshot_ids
       FROM research_job_preflights
       WHERE workspace_id = $1 AND id = $2
       FOR SHARE`, [input.workspaceId, input.preflightId]);
        const preflight = preflightResult.rows[0];
        if (!preflight)
            throw new SourceTaskPersistenceError('SOURCE_TASK_PREFLIGHT_NOT_FOUND', `Preflight ${input.preflightId} was not found.`);
        ...
        const snapshotResult = await client.query(`SELECT id, source_task_id, request_id, source_key, capability_version,
              connector_key, connector_version, policy_id, policy_version,
              decision, request, admission
       FROM source_admission_snapshots
       WHERE workspace_id = $1 AND id = $2
       FOR SHARE`, [input.workspaceId, input.admissionSnapshotId]);
        const snapshot = snapshotResult.rows[0];
        if (!snapshot)
            throw new SourceTaskPersistenceError('SOURCE_TASK_ADMISSION_NOT_FOUND', `Admission snapshot ${input.admissionSnapshotId} was not found.`);
        ...
        const inserted = await client.query(`INSERT INTO source_tasks (
       ...
       RETURNING id`, [
        ...
        if (inserted.rows[0]?.id)
            return true;
        const existing = await client.query('SELECT workspace_id, research_job_id, preflight_id, admission_snapshot_id, job_run_id, work_unit_id, max_attempts, effective_budget FROM source_tasks WHERE id = $1', [input.sourceTaskId]);
    ...
    if (!state)
        throw new Error('Source task disappeared after persistence.');
    return { created, state };
}
async function claimSourceTask(pool, input) {
    assertIdentifier(input.workspaceId, 'workspaceId', 'SOURCE_TASK_INPUT_INVALID');
    assertIdentifier(input.sourceTaskId, 'sourceTaskId', 'SOURCE_TASK_INPUT_INVALID');
    assertIdentifier(input.workerId, 'workerId', 'SOURCE_TASK_INPUT_INVALID');
    const state = await getSourceTaskState(pool, input.workspaceId, input.sourceTaskId);
    if (!state)
        throw new SourceTaskPersistenceError('SOURCE_TASK_NOT_FOUND', `Source task ${input.sourceTaskId} was not found.`);
    return (0, jobs_1.claimWorkUnit)(pool, state.workUnitId, input.workerId, input.expectedAttempt, input.leaseSeconds ?? 30);
}
async function recordSourceTaskUsage(pool, input) {
    ...
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        const task = await client.query('SELECT effective_budget, work_unit_id FROM source_tasks WHERE workspace_id = $1 AND id = $2 FOR UPDATE', [input.workspaceId, input.sourceTaskId]);
        const row = task.rows[0];
        if (!row)
            throw new SourceTaskPersistenceError('SOURCE_TASK_NOT_FOUND', `Source task ${input.sourceTaskId} was not found.`);
        const work = await client.query('SELECT status FROM job_work_units WHERE id = $1 FOR UPDATE', [row.work_unit_id]);
        ...
        const existing = await client.query('SELECT workspace_id, source_task_id, requests::text, pages::text, bytes::text, currency_micros::text, runtime_ms::text, metadata, occurred_at FROM source_task_usage_events WHERE id = $1', [input.eventId]);
        const existingRow = existing.rows[0];
        if (existingRow) {
            const same = existingRow.workspace_id === input.workspaceId &&
        ...
        const totals = await client.query(`SELECT COALESCE(sum(requests),0)::text AS requests,
              COALESCE(sum(pages),0)::text AS pages,
              COALESCE(sum(bytes),0)::text AS bytes,
              COALESCE(sum(currency_micros),0)::text AS currency_micros,
              COALESCE(sum(runtime_ms),0)::text AS runtime_ms
       FROM source_task_usage_events
       WHERE workspace_id = $1 AND source_task_id = $2`, [input.workspaceId, input.sourceTaskId]);
        ...
        if (!Number.isSafeInteger(next.requests) || next.requests > budget.maxRequests ||
        ...
        await client.query(`INSERT INTO source_task_usage_events (
...
async function recordSourceTaskFailure(pool, input) {
    assertIdentifier(input.workspaceId, 'workspaceId', 'SOURCE_TASK_INPUT_INVALID');
    assertIdentifier(input.sourceTaskId, 'sourceTaskId', 'SOURCE_TASK_INPUT_INVALID');
    ...
    return (0, jobs_1.recordWorkFailure)(pool, state.workUnitId, input.errorClass, input.errorCode, input.retryDelayMs ?? 0);
}
async function cancelSourceTask(pool, workspaceId, sourceTaskId) {
    const state = await getSourceTaskState(pool, workspaceId, sourceTaskId);
    if (!state)
        throw new SourceTaskPersistenceError('SOURCE_TASK_NOT_FOUND', `Source task ${sourceTaskId} was not found.`);
    if (state.status === 'cancelled')
        return 'cancelled';
    const status = await (0, jobs_1.requestWorkCancellation)(pool, state.workUnitId);
    if (!status)
        throw new SourceTaskPersistenceError('SOURCE_TASK_STATE_INVALID', `Source task ${sourceTaskId} is already terminal and cannot be cancelled.`);
    return status;
}
async function completeSourceTask(pool, input) {
    assertIdentifier(input.workspaceId, 'workspaceId', 'SOURCE_TASK_INPUT_INVALID');
    assertIdentifier(input.sourceTaskId, 'sourceTaskId', 'SOURCE_TASK_INPUT_INVALID');
    ...
    if (input.resultRef !== undefined)
        assertIdentifier(input.resultRef, 'resultRef', 'SOURCE_TASK_INPUT_INVALID');
    return (0, client_1.withPgTransaction)(pool, async (client) => {
        const task = await client.query(`SELECT job_run_id, work_unit_id, admission_snapshot_id, connector_key, connector_version,
              source_key, policy_id, policy_version
       FROM source_tasks WHERE workspace_id = $1 AND id = $2 FOR UPDATE`, [input.workspaceId, input.sourceTaskId]);
        const row = task.rows[0];
        if (!row)
            throw new SourceTaskPersistenceError('SOURCE_TASK_NOT_FOUND', `Source task ${input.sourceTaskId} was not found.`);
        const effectData = {
        ...
        if (status === 'succeeded') {
            const existingEffect = await client.query('SELECT data FROM job_effects WHERE work_unit_id = $1 AND effect_key = $2', [row.work_unit_id, exports.SOURCE_EXECUTION_RESULT_EFFECT]);
       ...
       RETURNING id`, [row.work_unit_id, exports.SOURCE_EXECUTION_RESULT_EFFECT, JSON.stringify(effectData)]);
       ...
       SET status = 'succeeded', completed_at = now(), lease_expires_at = NULL,
           ...
           updated_at = now()
       WHERE id = $1`, [row.work_unit_id]);
        const pending = await client.query(`SELECT count(*)::int AS count FROM job_work_units
       WHERE job_run_id = $1 AND status <> 'succeeded'`, [row.job_run_id]);
        if ((pending.rows[0]?.count ?? 1) === 0) {
            await client.query(`UPDATE job_runs SET status = 'succeeded', completed_at = now(), updated_at = now() WHERE id = $1`, [row.job_run_id]);
        }
        return { effectCreated: Boolean(effect.rows[0]) };
    });
}
async function getResearchJobPreflightState(pool, workspaceId, preflightId) {
    const preflight = await getResearchJobPreflight(pool, workspaceId, preflightId);
    if (!preflight)
        return null;
    const tasks = await pool.query(`SELECT id FROM source_tasks
     WHERE workspace_id = $1 AND preflight_id = $2
     ORDER BY created_at ASC, id ASC`, [workspaceId, preflightId]);
    const states = [];
...
//# sourceMappingURL=source-task-persistence.js.map